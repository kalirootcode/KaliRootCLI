            ...
            self.phases = [
                {
                    ...
                    "tasks": ["Definir alcance", "Establecer timeline"],
                    "status": "pending",
                },
                {
                    "name": "Fase 2: Ejecución",
                    "tasks": ["Implementar", "Probar"],
                    "status": "pending",
                },
                {
                    "name": "Fase 3: Revisión",
                    "tasks": ["Analizar resultados", "Documentar"],
                    "status": "pending",
                },
            ]
...
PROJECT_STRUCTURES: Dict[str, ProjectStructure] = {
    "pentest": {
        "dirs": [
            "01_recon",
            "02_scan",
...
class AgentFileManager:
    ...
    def run_task(self, instruction: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not files_map and not current_file:
                ...
                return {
                    "success": False,
                    "error": "No files parsed from AI response. Format error.",
                }
    ...
    def create_project_structure(
        ...
        try:
            structure: Dict[str, Any] = PROJECT_STRUCTURES[project_type]

            # Create project directory
            os.makedirs(project_dir, exist_ok=True)

            # Create subdirectories
            dirs: List[str] = structure["dirs"]
            for d in dirs:
                os.makedirs(os.path.join(project_dir, d), exist_ok=True)

            # Create files from templates
            files: Dict[str, str] = structure["files"]
            for filename, content_template in files.items():
                ...
                content = content_template.format(
                    name=project_name, name_lower=safe_name, description=description
                )
    ...
    def analyze_project_context(self, path: str | None = None) -> str:
        """
    ...
    def parse_natural_language_intent(self, prompt: str) -> Dict[str, Any]:
        ...
        if (
            ...
            # Detect type
            p_type = "tool"
            if "bot" in prompt or "telegram" in prompt:
                p_type = "tool"
            elif "pentest" in prompt:
                p_type = "pentest"
            elif "audit" in prompt or "auditoría" in prompt:
                p_type = "audit"
            elif "ctf" in prompt:
                p_type = "ctf"
    ...
    def delete_project(
        ...
        project_path = os.path.join(
            self.base_dir, project_name.lower().replace(" ", "_")
        )
        ...
        try:
            ...
            return {"success": True, "message": f"Deleted project: {project_name}"}
        except Exception as e:
            return {"success": False, "error": str(e)}
...
class ProjectPlanner:
    ...
    def __init__(self, file_manager: AgentFileManager | None = None):
    ...
    def create_plan(
        ...
        result = self.file_manager.create_from_template(
            "project_plan", name, description, objectives=obj_text, phases=phases_text
        )
    ...
    def create_audit_report(
        self, name: str, description: str, directory: str | None = None
    ) -> Dict[str, Any]:
        """Create a security audit report template."""
        result = self.file_manager.create_from_template(
            "security_audit", name, description, directory=directory
        )
...
def create_script(
    name: str, description: str = "", script_type: str = "python"
) -> FileCreationResult:
...
def create_plan(name: str, description: str, objectives: List[str]) -> Dict[str, Any]:
